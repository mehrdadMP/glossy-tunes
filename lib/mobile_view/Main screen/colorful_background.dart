...
class _ColorfulBackgroundState extends State<ColorfulBackground> {
  ...
  @override
  void initState() {
    bottomColor = widget.backgroundColors[widget.backgroundColors.length <= 1
  ...
  Widget build(BuildContext context) {
    Future.delayed(widget.duration, () {
      if (mounted) {
        setState(() {
          bottomColor =
              widget.backgroundColors[index % widget.backgroundColors.length];
        });
      }
    });
    ...
    return SizedBox(
      ...
      child: Stack(
        children: [
          AnimatedContainer(
            ...
            onEnd: () {
              setState(() {
                ...
                bottomColor = widget
                    .backgroundColors[index % widget.backgroundColors.length];
                topColor = widget.backgroundColors[
                    (index + 1) % widget.backgroundColors.length];
              });
            },
          ...
          for (int i = 0; i < widget.decoratorsList.length; i++)
            widget.decoratorsList[i],
          Positioned.fill(
              right: 0,
              left: 0,
              bottom: 0,
              top: 0,
              child: FittedBox(child: widget.child ?? const SizedBox())),
        ],
      ),
    );
  }
